...
const submitOrderSchema = z.object({
  ...
  userTokenA: z.string().optional(),
  userTokenB: z.string().optional(),
  relayerDummy: z.string().optional(),
...
app.post("/api/v1/orders", async (req, res) => {
  try {
    ...
    if (hasDummyRelayer && params.relayerDummy) {
      const dummyRelayerPubkey = new PublicKey(params.relayerDummy);
      ...
      if (transaction instanceof Transaction) {
        ...
        const userSigInfo = transaction.signatures.find(sig => 
          sig.publicKey.equals(userPubkey) && sig.signature !== null
        );
...
app.post("/api/v1/orders2", async (req, res) => {
  try {
    ...
    res.json({
      ...
      fee: result.fee,
      message:
        "Transaction built and partially signed by relayer. Please sign with wallet and broadcast.",
    });
  } catch (error) {
...
app.get("/api/v1/pools/:poolId/price", async (req, res) => {
  try {
    ...
    const tokenAUiAmount =
      tokenABalance / Math.pow(10, poolConfig.tokenADecimals);
    const tokenBUiAmount =
      tokenBBalance / Math.pow(10, poolConfig.tokenBDecimals);
    ...
    const response = {
      ...
      price: {
        [`${poolConfig.tokenASymbol}Per${poolConfig.tokenBSymbol}`]:
          tokenAPerTokenB.toFixed(6),
        [`${poolConfig.tokenBSymbol}Per${poolConfig.tokenASymbol}`]:
          tokenBPerTokenA.toFixed(6),
      },
      liquidity: {
  ...
  try {
    ...
    if (now - lastRequest < config.airdropRateLimitMs) {
      const remainingTime = Math.ceil(
        (config.airdropRateLimitMs - (now - lastRequest)) / 1000
      );
    ...
    if (tokenType === "SOL") {
      ...
      try {
        const relayerBalance = await connection.getTokenAccountBalance(
          relayerTokenAccount
        );
      ...
      // Get new balance
      const newBalanceResponse = await connection.getTokenAccountBalance(
        recipientTokenAccount
      );
      const newBalance = parseFloat(
        newBalanceResponse.value.uiAmountString || "0"
      );
