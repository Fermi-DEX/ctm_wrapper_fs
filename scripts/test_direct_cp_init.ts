...
async function main() {
  console.log('=== Direct CP-Swap Pool Initialization Test ===\n');
  ...
  try {
    ...
    console.log('Token A created with 9 decimals');
    ...
    console.log('Token B created with 6 decimals');

    // === Step 3: Derive PDAs ===
    console.log('\n=== Step 3: Deriving CP-Swap PDAs ===');
    ...
    console.log('\n=== Step 4: Creating Token Accounts and Minting ===');
    ...
    console.log('User Token 0:', userToken0.toBase58());
    console.log('User Token 1:', userToken1.toBase58());
    ...
    // Mint tokens based on decimals
    const amount0 = cpSwapPDAs.sortedToken0.equals(tokenAMint.publicKey)
      ? 1000 * 10 ** 9  // Token A has 9 decimals
      : 1000 * 10 ** 6; // Token B has 6 decimals

    const amount1 = cpSwapPDAs.sortedToken1.equals(tokenBMint.publicKey)
      ? 1000 * 10 ** 6  // Token B has 6 decimals
      : 1000 * 10 ** 9; // Token A has 9 decimals
    ...
    // === Step 5: Initialize Pool Directly ===
    console.log('\n=== Step 5: Initializing Pool Directly ===');

    // Initial amounts based on token decimals
    const initAmount0 = cpSwapPDAs.sortedToken0.equals(tokenAMint.publicKey)
      ? new BN(100 * 10 ** 9)  // 100 tokens with 9 decimals
      : new BN(100 * 10 ** 6); // 100 tokens with 6 decimals
    ...
    // Build instruction data for CP-Swap initialize with standard authority
    ...
    const initPoolAccounts = [
      ...
      { pubkey: cpSwapPDAs.cpSwapAuthority, isSigner: false, isWritable: false }, // authority
      ...
      { pubkey: userLpToken, isSigner: false, isWritable: true }, // creator_lp_token
    ...
    console.log('\n✅ SUCCESS! Pool initialized directly!');
